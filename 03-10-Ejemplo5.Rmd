...
Vamos a partir de la identificación obtenida por `seas` (que es similar a la de `auto.arima` eliminando uno de los coeficientes no significativos), incluyendo las tres variables ficticias que afectan un único mes: $log(DefEnfCer) \sim ARIMA_{12}(1,1,1)(1,1,1) + AI$.
...
Por defecto Arima solo puede estimar modelos que sean estacionarios e invertibles. Para verificarlo en este caso, puedes obtener gráficamente el inverso de la raíces de los polinomios característicos (que deben caer dentro del círculo unidad) con `plot(DefEnfCerArima2)`.
...
```{r}
ancho <- max(nchar(names(coef(DefEnfCerArima2)))) + 2
for(i in 1:length(coef(DefEnfCerArima2))) {
  wt <- wald.test(b = coef(DefEnfCerArima2), 
                  Sigma = vcov(DefEnfCerArima2), 
                  Terms = i)
  cat("\nCoeficiente: ", format(names(coef(DefEnfCerArima2))[i], width = ancho), "valor de p: ",
...
La figura 5 revela que Arima siempre es superior a Alisado en calidad de predicciones, con un error porcentual inferior entre 4 y 8 décimas según el horizonte temporal. Además, en el modelo Arima para horizontes de predicción superiores a 4 meses el error se mantiene prácticamente constante.
