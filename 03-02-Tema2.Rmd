...
* __Previsión $h$ periodos adelante__, como la previsión de la serie para el periodo $t+h$ disponiendo de información hasta el periodo $t$, y se denota por $\widehat{y}_{t+h|t}$. Por simplicidad lo escribiremos también como $\widehat{y}_{t+h}$.

\

* Así, $\widehat{y}_{t+1|t}$ es la __previsión un periodo adelante__ o a un periodo vista. Es decir, la previsión de la serie en $t+1$ desde el periodo $t$.

\

* De nuevo, por simplicidad denotaremos a $\widehat{y}_{t+1|t}$ como $\widehat{y}_{t+1}$; y como $\widehat{y}_{t}$ a la previsión en $t$, con datos hasta el periodo $t-1$ ($\widehat{y}_{t} = \widehat{y}_{t|t-1}$).

Se define como __error de previsión (intra-muestral) a un periodo vista__ a 
$$\widehat{e}_t=y_t-\widehat{y}_t,$$
\noindent de forma que la serie $\{\widehat{e}_t\}_{t=1}^T$ nos permitirá definir varios criterios de calidad de ajuste. 
  
\

## Medidas de precisión de la predicción
 
Dada una serie $\{y_t\}_{t=1}^T$, un método de predicción y su vector de errores (intra-muestrales a un periodo vista) asociado $\{\widehat{e}_t\}_{t=1}^T$, podemos definir múltiples medidas de calidad o precisión del método de predicción. Las más habituales son (siglas en inglés):

* Error medio (ME): $\frac{1}{T}\sum_{t=1}^T \widehat{e}_t$

\

* __Raíz del error cuadrático medio (RMSE)__: $\sqrt{\frac{1}{T}\sum_{t=1}^T \widehat{e}^2_t}$

\

* Error absoluto medio (MAE): $\frac{1}{T}\sum_{t=1}^T |\widehat{e}_t|$

\

* Error porcentual medio (MPE): $\frac{100}{T}\sum_{t=1}^T \frac{\widehat{e}_t}{y_t}$

\

* __Error porcentual absoluto medio (MAPE)__: $\frac{100}{T}\sum_{t=1}^T \big|\frac{\widehat{e}_t}{y_t}\big|$

\

* Error porcentual absoluto medio simétrico (sMAPE): $\frac{200}{T}\sum_{t=1}^T \Big|\frac{\widehat{e}_t}{y_t + \widehat{y}_t}\Big|$

\

* Error escalado absoluto medio (MASE): $\big(\frac{1}{T}\sum_{t=1}^T |\widehat{e}_t|\big)/q$, donde $q$ es el error absoluto medio para un método ingenuo de predicción:
...
* Correlación entre $\widehat{e}_t$ y $\widehat{e}_{t-1}$ (ACF1).

\

ME y MAE no permiten valorar la calidad de un modelo, pero si el posible sesgo de las predicciones.

* Lo esperado es un valor cercano a cero (relativo al valor medio de la serie). Valores muy alejados de cero son indicadores de sesgo de predicción.
...
Estos indicadores de calidad, por basarse en predicciones intra-muestrales a un periodo vista, adolecen de dos problemas. Primero, evalúan el error de predicción a un periodo vista, cuando en muchas situaciones reales las predicciones se realizan sobre un horizonte temporal más amplio. Segundo, son errores intra-muestrales, resultantes de predecir los mismos datos que se han usado para estimar los parámetros del método de predicción y, por tanto, sobre-estiman la capacidad predictiva del modelo.

Veremos durante el este tema métodos de evaluación de la calidad de las predicciones que superan estas limitaciones.
...
Veamos algunos métodos muy sencillos de predicción y sus funciones en el paquete `forecast`.

\

## Series _sin_ tendencia y _sin_ estacionalidad

Método de la Media: $\widehat{y}_{T+h}=(y_1+\ldots,y_T)/T$.
...
Método ingenuo I: $\widehat{y}_{T+h}=y_T$.
...
Método ingenuo II: $\widehat{y}_{T+h}=y_T + h(y_T-y_{T-1})$.

* La predicción $h$ periodos adelante es la __última observación__ disponible más $h$ veces el __último incremento__ observado. 
* No tiene función en `R`.
      
Método de la deriva: $\widehat{y}_{T+h}=y_T+h\frac{y_T - y_1}{T-1}$.
...
Método ingenuo con estacionalidad: $\widehat{y}_{T+h}=y_{T-m(k+1)}$.
...
* La línea en rojo es la previsión _extra_-muestral a largo plazo: $\widehat{y}_{T+h}=l_T$, donde $T=2011$. Observa que todas las previsiones están por encima del valor real de la serie.
