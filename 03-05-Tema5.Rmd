...
La figura 1 muestran una simulación del proceso $AR_{4}(2)$ definido por $y_t = 0.7y_{t-4} - 0.7y_{t-8} + \varepsilon_t$, y la figura 2 simula el proceso $AR_{12}(1)$ definido por $y_t = 0.6y_{t-12} + \varepsilon_t$. En ambos casos la muestra es de tamaño 500 y $\varepsilon_t$ se distribuye como una normal con media cero y varianza la unidad. (Todas las simulaciones se han realizado con la función `arima.sim` de la librería `stats`.)
...
La figura 3 muestra una simulación del proceso $MA_{4}(2)$ definido por $y_t = 0.7\varepsilon_{t-4} - 0.7\varepsilon_{t-8} + \varepsilon_t$, y la figura 4 simula el proceso $MA_{12}(1)$ definido por $y_t = 0.6\varepsilon_{t-12} + \varepsilon_t$. En ambos casos la muestra es de tamaño 500 y $\varepsilon_t$ se distribuye como una normal con media cero y varianza la unidad.
...
$$ y_t = c + \phi_m y_{t-m} + \phi_{2m} y_{t-2m} + ... + \phi_{Pm} y_{t-Pm}  + 
  ...
  \theta_{Qm} \varepsilon_{t-Qm},$$
\noindnet que usando el operador retardo queda
...
La figura 5 muestra una simulación de tamaño 500 para el proceso $ARMA_7(1,1)$ definido por $y_t = 0.7y_{t-7} - 0.5\varepsilon_{t-7} + \varepsilon_t$.
...
__Si la serie $y_t$ no es estacionaria en su parte estacional, pero tras diferenciarla $D$ veces se hace estacionaria, diremos que la serie es integrada estacionalmente de orden $D$__: $y_t \sim I_m(D)$.

* Por tanto, una serie estacionaria estacionalmente se indicará como $y_t \sim I_m(0)$.
...
La figura 6 muestra una simulación de tamaño 1000 para el proceso $ARIMA_{12}(0,1,1)(0,1,1)$ definido por $(1-L)(1-L^{12})y_t = (1+ \theta_1L)(1 + \theta_{12}L^{12})\varepsilon_t$. Este proceso se denomina _modelo de las aerolíneas_ por ser el PGD de muchas series mensuales de transporte de pasajeros, en concreto la serie mensual de pasajeros de avión. En la simulación se ha supuesto que $\theta_1=0.7$ y $\theta_{12}=-0.5$.
...
Una alternativa `auto.arima` es la función `seas` de la librería `seasonal`. La función `seas` tiene como ventajas que también analiza la conveniencia de usar la transformación logarítmica, que identifica posibles efectos calendario y valores extremos, y que suele ser más parsimoniosa que `auto.arima`. Sus desventajas es que sólo se puede aplicar para series mensuales o trimestrales. Veamos que identificación ofrece `seas`:
```{r} 
summary(seas(nacimientos))
``` 

En primer lugar, la función identifica el modelo de las aerolíneas para la transformación logarítmica de Nacimientos. Este resultado está en consonancia con el alcanzado con la función `auto.arima`. Además, identifica un valor extremo en diciembre de 2010, un efecto calendario Semana Santa (que estudiaremos en detalle en el siguiente ejemplo) y un efecto calendario días laborables del mes (que podemos entender equivalente a nuestro efecto días del mes).
...
\noindent donde _AI_ recoge todas las variables de intervención.
...
* Se crea una variable de intervención para caso
...
$$(1 - L^{12})(1 - L)\log(nacimientos_t) = (1 + \theta_1 L)(1 + \theta_{12} L^{12} + + \theta_{24} L^{24})\varepsilon_t + AI.$$
Si sustituimos $(1 - L^{12})\log(nacimientos_t)$ por $TVAnacimientos_t$, la tasa de variación anual de los nacimientos, y desarrollamos queda
$$TVAnacimientos_t = TVAnacimientos_{t-1} + \theta_1 \varepsilon_{t-1} + \theta_{12} \varepsilon_{t-12} + \theta_{24} \varepsilon_{t-24} + \ldots + \varepsilon_{t}+$$
...
$$\widehat{TVAnacimientos}_t = TVAnacimientos_{t-1}  - 0.45\varepsilon_{t-1} - 0.62\varepsilon_{t-12} - 0.16\varepsilon_{t-24} + \ldots + $$
$$0.037\cdot DiasMes_t - 0.042\cdot d1206_t + 0.057\cdot d0408_t - 0.07\cdot d0111_t + 0.032\cdot d0616_t.$$
  
En cada mes __la tasa de variación anual de los nacimientos es la misma que la del mes pasado__ ($\widehat{TVAnacimientos}_t = TVAnacimientos_{t-1}$). Además, si la tasa de variación anual del mes previo fue anómala ($\varepsilon_{t-1}$ diferente de cero) o la tasa de variación anual de hace uno o dos años fue anómala ($\varepsilon_{t-12}$ o $\varepsilon_{t-24}$ diferente de cero), corregimos la previsión para compensar (términos en medias móviles).
    
Respecto de la intervención, los febreros bisiestos nacen un 3.7% más de bebés. Por alguna razón, en diciembre de 2006 hubo un 4.2% menos de nacimientos de lo esperado; en abril de 2008 hubo un 5.7% más de nacimientos de lo esperado; en enero de 2011 hubo un 7% menos de nacimientos de lo esperado; y en junio de 2016 hubo un 3.2% más de nacimientos de lo esperado.

### Predicción de la serie {-}

Una vez dado por válido el modelo podemos pasar a realizar predicciones para los próximos años. Hay que tener en cuenta que hay cinco variables de intervención, una de ellas un efecto calendario (Bisiestos), para las que debemos indicar qué valores tomarán en el periodo de predicción. Vamos a fijar el horizonte de predicción en cuatro años y mostrar los resultado numérica (solo para el primer año) y gráficamente (figura 11b).
...
autoplot(pnac.ar2, 
     ylab = 'Nacimientos',
     main = 'Figura 11b. Nacimientos (2000-2018) y predicción (2019-2022)') +
...
La exclusión del número de días del mes no mejora la calidad de ajuste del modelo sobre Nacimientos medios al día porque su efecto ya estaba recogido en la variable Bisiestos del modelo sobre Nacimientos. Pensemos que febrero es el único mes que puede tener un número diferente de días. Sin embargo, siempre que esté justificado, es conveniente probar si la inclusión en el modelo del número de días del mes o el número de días laborables, o el número de domingos u otros efectos calendario, más allá de los febreros bisiestos, mejora su ajuste y capacidad predictiva. Extenderemos estas ideas en el siguiente ejemplo.
...
autoplot(chocolate, 
         xlab = "", 
         ylab = "Toneladas",
         main = "Figura 12. Producción de Chocolate (Australia)")
```
...
La figura 13 muestra que solo la doble diferenciación regular y estacional consigue que la transformación logarítmica de Chocolate sea estacionaria y ergódica.
...
```{r, echo = FALSE}
grid.arrange(
  ggAcf(log(chocolate), lag = 48, xlab = "", ylab = "d = D = 0", main = "Figura 13. FAC"),
...
ggtsdisplay(diff(diff(log(chocolate), lag = 12)), lag = 48,
            main = "Figura 14. FAC y FACP para Chocolate (log)")
```

Analizando la FAC y FACP (figura 14), tenemos que:
...
Por último, la función `seas` (también con un poco de ayuda) identifica un proceso $ARIMA_{12}(1,1,1)(1,1,0)$, con un efecto Semana Santa (de ocho días) un efecto día de la semana y un efecto año bisiesto, que se asemejan al efecto días laborables.
...
Vamos a considerar como modelo de partida el identificado con `auto.arima` excluyendo el parámetro no significativo.
...
autoplot(error, series="Error",
         ...
         xlab = "",
         ylab = "Error",
         main = "Figura 15. Error + Intervención") +
  geom_hline(yintercept = c(-3, -2, 2, 3)*sderror, 
             colour = c("red", "green", "green", "red"), 
             lty = 2) + 
...
for(i in 1:length(coef(choco.ar2))) {
  wt <- wald.test(b = coef(choco.ar2), 
                  ...
                  Terms = i)
  cat("\nCoeficiente: ", names(coef(choco.ar2))[i], "\tvalor de p: ", 
...
```{r}
ggAcf(error, lag = 36, main = "Figura 17. FAC del error del modelo")
Box.test(error, lag = 2,type = "Ljung-Box")
Box.test(error, lag = 24,type = "Ljung-Box")
Box.test(error^2, lag = 2, type = "Ljung-Box")
Box.test(error^2, lag = 24, type = "Ljung-Box")
...
Si sustituimos $(1 - L^{12})\log(chocolate_t)$ por $TVAchocolate_t$, desarrollamos el modelo y sustituimos los parámetros por sus estimaciones, queda
$$\widehat{TVAchocolate}_t = TVAchocolate_{t-1} + 0.31(TVAchocolate_{t-1} - TVAchocolate_{t-2}) $$
...
autoplot(pchoco.ar2, 
         xlab = "",
         ylab = 'Toneladas',
         main = 'Figura 17. Chocolate (1958-1994) y predicción (1995-1998)') +
...
* Nacimientos (log):
  ...
  + MAPE ARIMA: $1.6\%$ - ARIMA(0,1,1)(0,1,2) + AI 

  + MAPE ETS  : $1.8\%$ - ets(A,Ad,A)

* Chocolate (log):

  + MAPE ARIMA:  $7.0\%$ - ARIMA(1,1,1)(0,1,1) + AI

  + MAPE ETS  : $8.1\%$ - ets(A,A,A)

Los modelos ARIMA al permitir la inclusión de intervención han mejorado los resultados de los modelos de alisado. En el caso de Chocolate, la mejora en la calidad del ajuste es superior al punto porcentual.
...
Vamos a aplicar los conceptos vistos de _validación cruzada_ para la serie de Nacimientos. Además, vamos a mejorar ligeramente la rutina vista en el Tema 2 para incluir la intervención.

\

## Ejemplo: Nacimientos

Recordemos que la serie va de enero de 2000 hasta diciembre de 2018, es un proceso $ARIMA_{12}(0,1,1)(0,1,2)$ con intervención en febreros bisiestos y otros meses identificados como atípicos.
...
ggplot() +
  ...
  ggtitle("Figura 18. Error de predicción (MAPE) según horizonte temporal") +
