...
Para que esta línea de razonamiento sea válida es necesario que el proceso verifique ciertas condiciones, que sea  __estacionario y ergódico__.
...
cbind("Original" = libros,
      "Dif. anual" = diff(libros)) %>%
  autoplot(facets = TRUE,
           xlab = "",
           ylab = "",
           main = "Figura 1. Títulos publicados") + 
...
La figura 2 muestra una serie que es estacionaria en media, pero no lo es en sentido amplio: al inicio de la serie hay una mayor variablidad en los datos que en la zona intermedia.
      
```{r, echo=FALSE}
dnac <- diff(nacimientos)
ggplot() +
  geom_line(aes(x = time(dnac), y = dnac)) + xlab("") + ylab("") + 
  ggtitle("Figura 2. Estacionariedad en media pero no en varianza") + 
  geom_ellipse(aes(x0 = 1981, y0 = 0, a = 6, b = 4500,  angle = 0), 
               linetype = "dashed", colour = "red", size = 1) +
  geom_ellipse(aes(x0 = 2002, y0 = 0, a = 4, b = 3000,  angle = 0), 
               linetype = "dashed", colour = "red", size = 1) + 
  annotate("text", x=1982, y=6500, label="Mayor varianza", colour = "red") +
  annotate("text", x=2002, y=4300, label="Menor varianza", colour = "red") +
...
cbind("Nacidos" = nacimientos,
  ...
  autoplot(facets = TRUE,
           xlab = "",
           ylab = "",
           main = "Figura 3. Nacimientos y diferencia del logaritmo de Nacimientos")
```
...
Un caso concreto de la diferenciación es la __diferenciación regular o diferenciación de orden uno__, que consiste en restar a la observación de un periodo la del periodo precedente:
$$\nabla y_t = y_t - y_{t-1}.$$
Usualmente, la serie diferenciada $\nabla y_t$ ya es estacionaria en media. Por ejemplo, en la figura 1 teníamos la serie Libros (panel superior) no estacionaria, y la serie Libros diferenciada regularmente que es estacionaria.
...
Si $\nabla y_t$ no es estacionaria, se diferencia (regularmente) una segunda vez para obtener una diferenciación de segundo orden:
...
En el tema 2 vimos el argumento `lambda`, que fijado a 0 indicaba que había que transformar la serie logaritmicamente. Veamos en detalle que hay detrás de este argumento.


### Transformación logarítmica {-}

Si la serie original no es estacionaria en varianza porque los datos crecen con el nivel de la serie, es posible obtener esta propiedad por medio de transformaciones simples.
...
cbind("Nacidos" = nacimientos,
  ...
  autoplot(facets = TRUE,
           xlab = "",
           ylab = "",
           main = "Figura 5. Serie Nacimientos y algunas transformaciones Box-Cox")
```
...
En la figura 5, el panel inferior muestra la transformación _óptima_ de Box-Cox para Nacimientos, la que hace que la serie sea lo más estacionaria en varianza posible. El valor usado es $\lambda= -0.58$.
...
Por último, ten presente que __si se estima y predice una serie transformada, luego hay que deshacer  la transformación para obtener la predicción de la serie original__.

\

## Diferencia, Logaritmo y Tasa de variación

La transformación $\nabla y_t$ se puede interpretar como variaciones en nivel, pero $\nabla^2 y_t$ no tiene ninguna interpretación. __Nunca hay que perder de vista la realidad de las observaciones__.
...
La figura 6 muestra para la serie Nacimientos la serie original (panel superior), las tasas de variación mensual y anual (paneles dos y tres, respectivamente) y la doble diferencia regular y estacional del logarítmo de nacimientos (panel inferior). Parece que la tasa de variación mensual de los nacimientos y la doble diferencia son, de todas las transformaciones probadas en este epígrafe, las más estacionarias en media y varianza.
...
cbind("Nacidos" = nacimientos,
  ...
  autoplot(facets = TRUE,
           xlab = "",
           ylab = "",
           main = "Figura 6. Nacimientos y diversas transformaciones")
```
...
Recuerda que en la figura 5 puedes ver la serie original, la transformación logarítmica y la transformación Box-Cox para el valor de $\lambda$ óptimo.
...
Ya hemos indicado que bajo ciertas hipótesis los momentos primero y segundo de la serie caracterizan perfectamente el proceso estocástico. En el contexto de series temporales, si se da la estacionariedad en media, el primer momento de la serie es constante y, por tanto, no informativo. _Son los segundos momentos de la serie (covarianzas) los que caracterizan el proceso estocástico_.
...
```{r, echo = FALSE}
grid.arrange(
  ggAcf(libros, lag = 10, main = "Figura 7. FAC para Libros", xlab = "", ylab = "Libros"),
  ggAcf(diff(libros), lag = 10, main = "", ylab = expression(nabla*"Libros")),
  nrow = 2
)
```
...
grid.arrange(
  ggAcf(nacimientos, lag = 48, main = "Figura 8. FAC para Nacimientos", xlab = "", ylab = expression(y[t])),
...
```{r, echo = FALSE}
grid.arrange(
  ggPacf(libros, lag = 10, main = "Figura 9. FACP para Libros", xlab = "", ylab = "Libros"),
  ggPacf(diff(libros), lag = 10, main = "", ylab = expression(nabla~"Libros")),
  nrow = 2
)
```
...
```{r, echo = FALSE}
grid.arrange(
  ggPacf(nacimientos, lag = 48, main = "Figura 10. FACP para Nacimientos", xlab = "", ylab = "Nacimientos"),
  ggPacf(diff(diff(nacimientos), lag = 12), lag = 48, main = "", ylab = expression(nabla*nabla[12]*"Nacimientos")),
  nrow = 2
)
```
...
Las funciones `tsdisplay` y `ggtsdisplay` muestran la serie temporal, su FAC y su FACP en un único gráfico (véase figura 11). 
```{r}
ggtsdisplay(diff(libros), main = "Figura 11. FAC y FACP para la Libros diferenciada")
```
...
También existen procesos con _estacionalidad estocástica_ del tipo $y_t = y_{t-m} + \varepsilon_t$, donde _m_ es el orden estacional.

* Contrastar la existencia de raíces unitarias estacionales es complicado y no lo veremos en este curso. (Véase [Testing for unit roots in seasonal time series: Some theoretical extensions and a Monte Carlo investigation](https://www.sciencedirect.com/science/article/abs/pii/0304407694900302))
...
* En ambos casos, si se rechaza la hipótesis nula es porque la serie tiene una raiz unitaria (tendencia estocástica) y habría que diferenciarla.
...
Además, en `forecast` está disponible la función `ndiffs` que estima el número de diferencias necesarias para que una serie sea estacionaria. Por defecto emplea el contraste KPSS, pero también están disponibles Dickey-Fuller y Phillips-Perron.
...
* Ghysels, E., Lee, H. S. y Noh, J. (1994) _Testing for unit roots in seasonal time series: Some theoretical extensions and a Monte Carlo investigation_. Journal of Econometrics, 62(2), pp. 415-442. doi.org/10.1016/0304-4076(94)90030-2

* Kwiatkowski, D., Phillips, P.C.B., Schmidt, P. y Shin, Y., (1992), _Testing the Null Hypothesis of Stationarity Against the Alternative of a Unit Root: How Sure Are We That Economic Time Series Have a Unit Root?_, Journal of Econometrics, 54, pp. 159–178. doi.org/10.1016/0304-4076(92)90104-Y

* Pfaff, B. (2008). _Analysis of Integrated and Cointegrated Time Series with R_. Colección Use R!, Ed. Springer.
