...
Observa como la inclusión de intervención modifica la auto-identificación, que ahora es un proceso ARIMA(1,2,0). Así, ahora tememos el modelo identificado por nosotros a partir de la FAC y la FACP ARIMA(0,2,1) y el modelo auto-identificado ARIMA(1,2,0). Un análisis de la calidad de las previsiones y del comportamiento del residuo indica que el segundo proceso es más adecuado. Por tanto, asumimos que $log(aforo_t) \sim ARIMA(1,2,0)$ con intervención.
...
La figura 22 muestra que para ningún año se observa un error atípico. Si se incluye una intervención asociada al residuo más elevado en 1974, no resulta significativa.
...
La hipótesis de normalidad se rechaza 5%. Además, la muestra es demasido pequeña como para poder aplicar el Teorema Central del Límite. Toda la inferencia realizada queda invalidada, no podemos afirmar si un coeficiente es o deja de ser significativo. Sin embargo, el modelo estimado sigue siendo válido con fines predictivos.
