...
![Aproximación Box-Jenkins](BoxJenkins.png)
...
Nuestro modelo estimado es: $\widehat{libros}_t = libros_{t-1}$
...
La figura 21b muestra que para ningún año se observa un error atípico. El residuo más elevado en 1974 si se incluye, no resulta significativo.
...
autoplot(error, series="Error",
         ...
         xlab = "",
         ylab = "Error",
         main = "Figura 21b. Error + Intervención") +
  geom_hline(yintercept = c(-3, -2, 2, 3)*sderror, 
             colour = c("red", "green", "green", "red"), 
             lty = 2) + 
  geom_point() +
...
Si sustituimos $(1 - L)^2 log(aforo_t)$ por $(1 - L) TVAaforo_t$, donde $TVAaforo$ es la tasa de variación anual del aforo, y desarrollando queda:
...
Respecto de la intervención, en 1979 hubo un 15% menos de vehículos de lo esperado, en 1981 y 1984 en torna a 8% más y en 2011 un 9% menos.
...
autoplot(parima120, 
     ylab = 'Vehículos (000)',
     main = 'Figura 22. Aforo (1960-2018) y predicción (2019-2023)') +
...
autoplot(alimentospc, 
         xlab = "", 
         ylab = "Kg per cápita",
         main = "Figura 23. Consumo alimentario en hogar")
```

### Transformación de la serie {-}

La figura 24 indica que la serie original ya es estacionaria. El contraste KPSS así lo confirma, por tanto asumimos que $d=0$ o $alimentospc_t \sim I(0)$.
...
```{r, echo = FALSE}
grid.arrange(
  autoplot(log(alimentospc), xlab = "Alimentos", ylab = "", main = "Figura 24. Consumo de alimentos"),
...
```{r} 
ggtsdisplay(alimentospc, main = "Figura 25. Consumo de alimentos per cápita")
```
...
Vamos a ver la gráfica de los residuos de este proceso para identificar rápidamente si hay valores extremos (figura 26).
...
autoplot(error, series="Error",
         ...
         xlab = "",
         ylab = "Error",
         main = "Figura 26. Error + Intervención") +
  geom_hline(yintercept = c(-3, -2, 2, 3)*sderror, 
             colour = c("red", "green", "green", "red"), 
             lty = 2) + 
  geom_point() +
...
\noindent que desarrollando queda:
...
autoplot(parima100, 
     ...
     main = "Figura 27. Consumo de alimentos y predicción") +
